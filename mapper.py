...
class _CFSubcluster:
    ...
    def __init__(self, n_samples=0, linear_sum=None):
    ...
    def merge_subcluster(self, nominee_cluster, threshold):
        ...
        # print(new_n, new_ls)
        # print(type(new_n), type(new_ls))
        new_centroid = (1 / new_n) * np.array(new_ls)
...
class Birch(
    ...
    * Roberto Perdisci
    ...
    def _fit(self, X, n_samples, partial):
        ...
        subcluster = _CFSubcluster(n_samples=n_samples, linear_sum=X)
        ...
        self.subcluster_centers_ = np.concatenate([leaf.centroids_ for leaf in self._get_leaves()])
    ...
    @_fit_context(prefer_skip_nested_validation=True)
    def partial_fit(self, X=None, y=None, n_samples=0):
        """
        ...
        if X is None:
            ...
            return self._fit(X, n_samples, partial=True)
...
def calculate_distance(c1, c2):
    n1, ls1, ss1 = c1.n_samples_, c1.linear_sum_, c1.squared_sum_
    n2, ls2, ss2 = c2.n_samples_, c2.linear_sum_, c2.squared_sum_
    term1 = ss1 / n1
    term2 = 2 * np.dot(ls1, ls2) / (n1 * n2)
    term3 = ss2 / n2
    d = np.max(np.sqrt(term1 - term2 + term3))
    return d
    
    
def rebuild(tree):
    global fixed
    total_leaf = len(tree._get_leaves())
    while total_leaf >= fixed:
        subclusters = tree._get_leaves()
        l = len(subclusters)
        np_arr = np.zeros((l, l))
        for i in range(l - 1):
            for j in range(i + 1, l):
                d = calculate_distance(subclusters[i].subclusters_[0], subclusters[j].subclusters_[0])
                np_arr[i, j] = np_arr[j, i] = abs(d)
        np_arr[np_arr <= 0] = np.inf
        min_distances = np.min(np_arr, axis=0)
        threshold = np.mean(min_distances)
        new_tree = Birch(n_clusters=tree.n_clusters, threshold=threshold, branching_factor=tree.branching_factor,
                         machines=tree.machines)
        for cluster in subclusters:
            new_tree.partial_fit(n_samples=cluster.subclusters_[0].n_samples_, X=cluster.subclusters_[0].linear_sum_)
        del tree
        tree = new_tree
        total_leaf = len(tree._get_leaves())
    return tree
...
def solve():
    global chunk_size, fixed
    machines = 1
    brc = Birch(n_clusters=None, threshold=0.01, machines=machines)
    arr = []
    ...
    #         arr.append(X)
    chunk_size = len(arr)
    fixed = ceil(chunk_size / machines)
    # print(chunk_size)
    # t = time.time()
    # print(t)
    for X in arr:
        brc.partial_fit(X=X, n_samples=1)
        total_leaf = len(brc._get_leaves())
        # print(total_leaf, ceil(chunk_size / brc.machines))
        # subclusters = brc._get_leaves()
        # print(len(subclusters))
        # for subcluster in subclusters:
        #     print(subcluster.subclusters_[0].n_samples_, subcluster.subclusters_[0].linear_sum_)
        if total_leaf >= fixed:
            # print(f"total_leaf: {total_leaf}, ceil(chunk_size / brc.machines): {ceil(chunk_size / brc.machines)}, {fixed}")
            brc = rebuild(brc)
            # print("here")
            # subclusters = brc._get_leaves()
            # # print(len(subclusters))
            # for subcluster in subclusters:
            #     print(subcluster.subclusters_[0].n_samples_, subcluster.subclusters_[0].linear_sum_)
            # print(f"threshold: {brc.threshold}")
    # print(time.time() - t)
    subclusters = brc._get_leaves()
    # arr = []
    for subcluster in subclusters:
        print(1, end=" ")
        print(*subcluster.subclusters_[0].centroid_)
        # arr.append(subcluster.subclusters_[0].centroid_)
    # print(subclusters[0].subclusters_[1].n_samples_, subclusters[0].subclusters_[1].linear_sum_,
    #       subclusters[0].subclusters_[1].centroid_)
    # brc.n_clusters = 4
    # brc._global_clustering()
    # reducer(arr)
    # print(time.time() - t)
    # print(brc.predict(arr))
